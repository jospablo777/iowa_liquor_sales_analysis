...
I've written an R package to facilitate the download from the Iowa Open Data Portal; Please check [iodDownloader GitHub page](https://github.com/jospablo777/iodDownloader) for more details on the data pull. The package can be installed with:
...
We create a vector with only the items available several years before pandemic. Seting the new product filter in 2016 should be anough.
...
The mean difference between the estimated parameters distribution is `r round(mean(BESTout$mu1 - BESTout$mu2)/1000, 1)` sold bottles. Also, note we set 'compVal' as 5% of pre-pandemic sales. This comparison value is also called the region of practical equivalence (ROPE), a value of difference good enough for practical purposes. We have observed that the ROPE we set up (which denotes an increase of at least 5% in the number of bottles sold) does not reach the HDI range. The ROPE left behind the HDI indicates that alcohol sales have, in fact, went up by more than 5% during the pandemic. It is worth noting that the ROPE serves as a boundary around the null value (which is zero, meaning no observable effect).

Now, let's look how many times (in average) the alcohol consumption increased. If you want to learn more about ROPE interpretation and this kind of inference, please revisit [-@kruschke2018].
...
Here, we explored REAL data made publicly available by the Iowa Government. We did lots of data wrangling, time series analysis, and visualizations, then concluded with some statistical inference. Now, we know a little more about alcohol consumption dynamics and how to generate new information in the field of alcohol retail sales.
