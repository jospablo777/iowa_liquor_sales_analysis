...
```{r}
day_with_more_sales <- sales_in_covid_times %>% 
  filter(bottles_sale_per_million == max(bottles_sale_per_million))

paged_table(day_with_more_sales) 
```
...
And there you have it! It is still Dec 2020. So we can rule out hte hypothesis of the decrease in sales due to the exclusion of new products while the old ones were drop into the oblivion. If a product is good and sells well, it should remain into the market


### Now back to our `sales_in_covid_times` data set

We will formaly test if there was an increase in the alcohol sales during the pandemic, and will also try to quantify the effect
...
We will use a Bayesian approac...
