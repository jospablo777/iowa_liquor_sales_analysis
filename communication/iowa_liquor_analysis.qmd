---
title: "Iowa Liquor Sales: 2.0 You are (Not) in a Pandemic"
author: 
...
You can take this as a history I tell with the data at hand, but also as a tutorial of the thing that can be done using data. So please do not hesitate in using the code below in your professional/personal projects...

## About the data set

The data set contains the purchase of spirits with a class "E," which means purchases made by grocery, liquor, and convenience-like stores (also including supermarkets). We will work with the "Iowa Liquor Sales" data set provided by the Iowa Open Data Platform. This data consist mostly in sales, but also includes geo-spatial data, and item descriptions. The data is constantly updated and licensed under CC, so the use is less restrictive than other kinds of data. Some trade-offs are that purchases of beverages like beer, wine, or moonshine are not included, so we do not have a full scope of alcohol consumption.

If you run this analysis I recommend you to use a machine with at least 32 GB of RAM, since the fresh downloaded data is around 12 GB. No GPU will be needed.
...
After you finish the experiment and present your final results and strategies, take that list, prioritize, and work on your next project. For the prioritizing, I recommend you put the most interesting ones at the top of the list; after all, we're here for fun. If that doesn't get your ideas approved, prioritize the ones with higher revenue-generating potential.
...
For a comprehensive review on the subject, check Kruschke [-@KruschkeLiddell2017]. It is convenient for a data scientist to understand how this technique works, so it is worth taking a couple of hours of your time for this read.

For this test we will need to install the `BEST` library, but it was brought down from CRAN some time ago. No worries, you can install it with the following code.
...
This method relies on MCMC to estimate the posterior distribution of the population parameter we are interested in, the monthly mean sales of liquor bottles. So, the first step will be to run the MCMC to get the posterior distributions; we will do this with the function `BESTout`.
...
We will do our inference with visual aids, but you can explore the object `BESTout` if you are curious since it contains lots of information. In the following chunk, we plot our posteriors. Kruschke's library includes a lot of valuable functions. To get the best charts, we will use `plotPost`.
...
Here, we can appreciate the posterior distribution for our parameters, the mean sales during the pandemic, and the mean sales in a pre-pandemic time. The black bars at the base of the distributions are the highest density intervals (HDI), also called the credible intervals, which are the distribution regions with the most credible values for the parameter. First, we can notice that the estimated distribution for the pandemic parameter is further to the right. At the same time, the pre-pandemic param fell behind, hinting that the average sales during the pandemic were higher. Second, the HDIs do not overlap.
...
The mean difference between `r mean(BESTout$mu1 - BESTout$mu2)`
Now, let's look how many times the alcohol consumption increased. If you want to learn more about ROPE interpretation and this kind of inference, please revisit [-@Kruschke2018]
