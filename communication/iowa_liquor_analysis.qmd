...
If you plan on running an analysis using this dataset, I recommend using a machine with at least 32 GB of RAM, as the data is around 12 GB in size. No GPU will be required for this task.
...
How many locations are missing? This is important since we will do some geospatial analysis later tutorials.
...
The `county_number` and `store_location` are the variables with the most missing values. Around `r round(missing_location/1e6, 2)` M of invoices do not have location. Later we will deal with this issue, in the next analysis.
...
Please interact with these charts by hovering over the elements. First, we can notice that the stores avoid making purchases on weekends. Then we see that October and December are the months with the most purchases in the year, the same for the last quarter of the year. And finally, it seems that 2013 and 2020 were good years.
...
* The seasonal pattern through the years
* The seasonal pattern through the weeks
...
![Photograph by Dustintitus, distributed under a CC-BY 4.0 license.](images/CHV_Original.tif.jpg)
