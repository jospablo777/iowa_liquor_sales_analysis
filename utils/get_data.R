...
# ----------------------------------------------------------------
# Generalform to download data in batches
download_iowa_data <- function(data_id, data_name, total_of_rows=28176372, batch_size=1000000) {
  # to the date, we have a total of 28176372 rows
  ...
  for (i in 1:n_iterations) {
    ...
    # we will save our data locally to save time
    file_name <- paste0('data/', data_name, i, '.csv')
    write_csv(df,file_name)
...
read_iowa_data <- function(folder_path='data', data_name) {
  filenames <- list.files(folder_path, 
                          pattern=paste0(data_name, ".*\\.csv$"), # Regex to match data_nameXXX.csv
                          full.names=TRUE)
